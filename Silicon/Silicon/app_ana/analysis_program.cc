...
int main(int argc,char**argv) {
  ...
  if (dr.setup()->geo().getName()!="G4GeoSilicon/GeoSilicon")
    return 1;
  //energy_init=0.027 keV for 0.2 MeV neutrons, 0.095 for 0.7 MeV neutrons, 2.861 for 3 MeV neutrons, 5.756 for 6 Mev neutrons, 9.7 for 10 MeV neutrons
  ...
  silicon.addFilter(new GriffAnaUtils::SegmentFilter_Volume("RecordFwd"));
  ...
  auto h_recoil_PKA_once = hc.book1D("Recoil_spectra_PKA_once",nbins, 0, energy_init*1000, "Recoil_spectra_PKA_once");
  ...
  auto h_Daughter_PKA_once = hc.bookCounts("PKA_particle_type_once", "PKA_particle_type_once");
  ...
  h_recoil_PKA -> setXLabel("Energy [keV]");
  h_recoil_PKA -> setYLabel("Frequency [PKA-1 keV-1]");
  ...
  while (dr.loopEvents()) {
    ...
    while (auto track = ti.next()){
      if (track->pdgName()=="Si28"){
                ...
                others+=1;}
      if (track->startEKin()*1000>0.021 && (track->atomicNumber()==14 or track->atomicNumber()==13 or track->atomicNumber()==12)){
        h_recoil_PKA_high_Z ->fill(track->startEKin()*1000); 
        // printf("Particle daughter info %s %s %f keV \n",
        // track->pdgNameCStr(),track->creatorProcessCStr(),track->startEKin()*1000);
      }else if(track->startEKin()*1000>0.021 && (track->atomicNumber()<8 and track->atomicNumber()>0)){
        h_recoil_PKA_low_Z ->fill(track->startEKin()*1000);}
      if(track->startEKin()*1000>0.021 && track->atomicNumber()>0){ h_recoil_PKA->fill(track->startEKin()*1000); }}
