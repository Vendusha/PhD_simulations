...
def analyze_event(filename,e, neighbours=9, r=4,par=0.05, method="dbscan"):
    X = []
    Y = []
    Z = []
    X_end = []
    ...
    event.clustering(track, track_vacancies, Energy, neighbours, r, par=par,method=method, showPlot=True)
    return(event)
n = 20
...
for file in os.listdir(folder):
    if file.endswith(".txt") and file[0]=="S":
        print(file)
        events += 1
        event = analyze_event((os.path.join(folder, file)), e, neighbours=neighbours, r=r, par=par, method=method)
